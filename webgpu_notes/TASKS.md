...
## CURRENT STATUS (updated after Round 24, 2026-09-11) — read this first
...
1. **[MEDIUM, found in Round 24 — now the blocker actually keeping SDFGI from rendering] `GiShaderRD:0:set0` binding 28: `None of the supported sample types (Float|UnfilterableFloat) of [Texture] match the expected sample types (Uint)`.** This is the *original* finding from this whole task's very first live run (Round 1's "three more real issues", finding 3) — never fixed, just eclipsed for 23 rounds by louder, earlier-firing errors. A binding whose actual bound texture's sample type is Float/UnfilterableFloat but whose BindGroupLayout expects `Uint` — the same general class of bug as Tasks 8.7-8.9, but nothing has instrumented which real texture is landing there or why the BGL expects Uint. **Next step**: same bind-group-entry-level instrumentation technique used for every other BGL/BindGroup mismatch this task has found — log the actual `WGTexture*`/format at the `UNIFORM_TYPE_IMAGE`/`UNIFORM_TYPE_STORAGE_BUFFER` bind-group-entry construction site in `uniform_set_create()` for `GiShaderRD` binding 28 specifically, and separately check why the BGL landed on `Uint` (likely a `wgsl_storage_tex_format`/`wgsl_tex_sample_type` scan reading an integer format off the WGSL text where the real GLSL declaration is actually a float/sampled type — or vice versa).

2. **[MEDIUM, found in Round 24, not yet investigated] `SceneForwardClusteredShaderRD` pipelines 11, 13 (variant 17) and 14, 15 (variant 8): `GPUValidationError: No attachment was specified` at `CreateRenderPipeline`, cascading into `Render pass has no attachments` and then every subsequent `CommandBuffer`/`BindGroup` for the rest of the session (`is invalid due to a previous error`, ~2,680 occurrences per 65s capture — this is almost certainly the single largest source of console noise in every recent capture, Round 24's included).** Not yet investigated at all — unclear whether this is SDFGI-specific (only two distinct shader variants affected) or a pre-existing, unrelated pipeline-construction gap that happened to start surfacing once earlier errors stopped masking it. **Next step**: find where `SceneForwardClusteredShaderRD`'s render pipeline descriptors are built (`rendering_device_driver_webgpu.cpp`'s `render_pipeline_create()` or equivalent) and check why variants 17 and 8 specifically end up with zero color/depth attachments — likely a framebuffer-format or attachment-list construction bug specific to whatever draw-call configuration variants 17/8 correspond to (check `scene_forward_clustered.glsl`'s mode defines for what distinguishes variant 8 and 17 from the others, none of which hit this).

3. **[LOW, found in Round 24, not yet investigated] `Dimension (TextureViewDimension::eND) of [TextureView] doesn't match the expected dimension` — one occurrence, texture/shader not yet identified.** Only a single hit per capture (vs. thousands for item 2), so likely narrow/rare. Not chased this round; needs its own repro/identification pass.
...
**Suggested next round priorities, in order**: (1) item 1 — the `GiShaderRD` Uint/Float sample-type mismatch, now the top blocker and this task's *original* first-ever finding (Round 1), (2) item 2 — the `SceneForwardClusteredShaderRD` "No attachment was specified" bug, since its cascading "invalid due to previous error" spam is the single largest source of console noise in every recent capture and may itself be masking still more, (3) once items 1-2 land, another fresh full capture to check what's left (same pattern every round since 19 has followed), (4) item 3 (the Dimension mismatch, likely quick once isolated), (5) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite — and while doing so, audit/fix the `extensions_support` export-preset gap Round 23 found so future scratch projects don't silently test a stale template, (6) items 8-9 as time allows.
