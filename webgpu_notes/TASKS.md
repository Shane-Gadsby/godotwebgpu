...
## CURRENT STATUS (updated after Round 25, 2026-09-11) — read this first
...
1. **[MEDIUM, found in Round 24, not yet investigated — now the sole documented blocker keeping SDFGI from rendering] `SceneForwardClusteredShaderRD` pipelines 11, 13 (variant 17) and 14, 15 (variant 8): `GPUValidationError: No attachment was specified` at `CreateRenderPipeline`, cascading into `Render pass has no attachments` and then every subsequent `CommandBuffer`/`BindGroup` for the rest of the session (`is invalid due to a previous error`, ~2,680 occurrences per 65s capture — confirmed by Round 25 to be the single largest source of console noise; total error count barely moved when Round 25's fix landed, entirely because of this item's spam).** Not yet investigated at all — unclear whether this is SDFGI-specific (only two distinct shader variants affected) or a pre-existing, unrelated pipeline-construction gap that happened to start surfacing once earlier errors stopped masking it. **Next step**: find where `SceneForwardClusteredShaderRD`'s render pipeline descriptors are built (`rendering_device_driver_webgpu.cpp`'s `render_pipeline_create()` or equivalent) and check why variants 17 and 8 specifically end up with zero color/depth attachments — likely a framebuffer-format or attachment-list construction bug specific to whatever draw-call configuration variants 17/8 correspond to (check `scene_forward_clustered.glsl`'s mode defines for what distinguishes variant 8 and 17 from the others, none of which hit this).

2. **[LOW, found in Round 24, re-identified in Round 25, not yet fixed] `GiShaderRD:2:set0` binding 2 (`sdf_cascades[]`): `Dimension (TextureViewDimension::e2D) of [TextureView of Texture (unlabeled 4x4 px, TextureFormat::RGBA8Unorm)] doesn't match the expected dimension (TextureViewDimension::e3D)`.** Only a single hit per capture (vs. thousands for item 1), so likely narrow/rare — probably fires for one unused SDFGI cascade slot (`sdf_cascades[SDFGI_MAX_CASCADES]`, a `texture3D` array in `gi.glsl`, binding 1 → WGSL binding 2) when fewer cascades are configured than `SDFGI_MAX_CASCADES`. **Strong lead from Round 25**: the 4×4 `RGBA8Unorm` placeholder texture is the exact same shape as Round 25's `voxel_gi_buffer` bug (a wrong-typed default-texture fallback) — check `gi.cpp`'s unused-cascade-slot fallback for this binding first, before assuming a new bug class; it may just need a 3D-shaped default (`DEFAULT_RD_TEXTURE_3D_WHITE`/`_BLACK`, already used correctly elsewhere in this same file for sibling cascade-array bindings) instead of whatever 2D default is landing here.
...
**Suggested next round priorities, in order**: (1) item 1 — the `SceneForwardClusteredShaderRD` "No attachment was specified" bug, now the sole documented blocker keeping SDFGI from rendering, since its cascading "invalid due to previous error" spam is the single largest source of console noise in every recent capture and may itself be masking still more, (2) item 2 — the `GiShaderRD` Dimension mismatch, worth checking against Round 25's "wrong-typed default fallback" bug shape first (a much smaller fix than a new investigation if it's the same pattern), (3) once items 1-2 land, another fresh full capture to check what's left (same pattern every round since 19 has followed) — this may finally be the round SDFGI actually renders, (4) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite — and while doing so, audit/fix the `extensions_support` export-preset gap Round 23 found so future scratch projects don't silently test a stale template, (5) items 8-9 (SsaoInterleave R8Unorm, canvas preferred-format warning) as time allows.
