...
## CURRENT STATUS (updated after Round 19, 2026-09-11) — read this first
...
1. **[HIGH — affects the user's actual current settings] SDFGI produces a silent, zero-console-error, solid-black render.** Root-caused as not-yet-understood in Round 17 (isolated: `sdfgi_enabled` alone, everything else off, is sufficient to reproduce). The render-pass equivalent of Round 16's read_write-storage-shadow-refresh bug was checked and ruled out (no render-stage shader anywhere in the engine uses an unqualified read_write image, confirmed by repo-wide grep). Completely uninvestigated since Round 17 — Rounds 18-19 focused on the per-feature bugs the user explicitly reported instead. **Next step**: same technique as the Round 16 DOF investigation — dump real `createShaderModule`/`createBindGroup` contents for SDFGI's compute shaders (`sdfgi_integrate.glsl`, `sdfgi_direct_light.glsl`, `sdfgi_preprocess.glsl`, `gi.glsl`) via the CDP/JS-hook technique and look for a *data-corruption* pattern (wrong texture bound, a shadow-copy gap like Round 16's, an uninitialized read), since there's no validation error to follow this time. This is the one actually blocking the user's real, currently-configured project.

2. **[LOW — doesn't reproduce under the user's actual per-feature settings] `SsaoInterleaveShaderRD`'s `dest_image` binding (set0/binding0) sometimes gets a real `R8Unorm` texture where the shader layout correctly expects `RGBA8Unorm`.** First seen in Round 17 (`ssr`+`ssao`+`ssil` enabled, `sdfgi` disabled), re-confirmed independently in Round 19 while stress-testing all seven non-SDFGI effects enabled simultaneously. Confirmed in Round 19 that this is **not** a DCE/WGSL-text problem (the binding is written unconditionally in every shader variant, so the existing text scan always finds `rgba8` correctly) and **not** caused by either of Round 19's fixes (verified: the new pre-DCE seeding map is function-local per shader-container call and only ever inserts-if-absent, never overwrites). The bug is therefore on the **engine/`ss_effects.cpp` side** — the wrong real texture is being selected/bound for some `downsample_mode` combination. **Next step**: bind-group-entry-level instrumentation — log the actual `WGTexture*`/format at the exact `UNIFORM_TYPE_IMAGE` bind-group-entry construction site in `uniform_set_create()` when this specific mismatch fires, cross-reference against `ss_effects.cpp`'s texture-selection logic for whichever `downsample_mode` is active in the repro. Not urgent: doesn't reproduce with one-effect-at-a-time testing, which is how the user actually configures their project today.

3. **[LOW, cosmetic/perf] Canvas configured with a non-preferred format.** Startup warning: `WebGPU canvas configured with a different format than is preferred by this device ("rgba8unorm")` — Dawn does an extra copy every frame to compensate. Flagged by the user's own "performance gains to be made" observation (Round 18) but not investigated. **Next step**: compare `navigator.gpu.getPreferredCanvasFormat()` against whatever this driver requests in `context.configure()` (`rendering_context_driver_webgpu.cpp`) and match them if the preferred format is usable.

4. **[VERY LOW, cosmetic] One-time `WebGPU: Unsupported DataFormat 1` warning** (`DATA_FORMAT_R4G4B4A4_UNORM_PACK16`, fires once during SDFGI startup, self-suppresses on repeat). `_data_format_to_wgpu()` has no mapping for this obscure packed format. Doesn't appear to block anything. Not investigated.

**Suggested next round priorities, in order**: (1) SDFGI black-screen (item 1 above — the only thing actually broken in the user's current real settings), (2) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite (e.g. one exported build per `Environment` toggle, checked into or generated fresh by CI) now that it's proven itself repeatedly — would turn every fix in this log into a permanent regression guard instead of a one-off manual verification, (3) items 2-4 above as time allows.
