...
11. **[HIGH, NOT ROOT-CAUSED, top priority] SDFGI's brightness/exposure climbs continuously over time with SDFGI enabled, until the 3D scene goes solid white — with zero console errors.** Reported directly by the user on their real machine, immediately after Round 27/28's milestone. Resets back to normal whenever any SDFGI field is changed (expected: a field change reallocates `gi.cpp`'s SDFGI cascades and clears the history/light textures — see `SDFGI::create()`), then climbs toward white again. This is a pure visual/algorithmic runaway, not a validation error.
    ...
    **Refined next hypothesis, not yet tested**: the packed RGBE9995 format's ~9-bit-mantissa quantization may be providing an *implicit damping/energy-dissipation effect* on the self-illumination feedback loop (bounce light read back next frame loses a small fraction of magnitude to rounding every round-trip, especially at high exponents where the quantization step size grows) that native float storage — lossless — doesn't have, allowing genuine unbounded growth. This would mean the real fix isn't a clamp at all but either an explicit decay/normalization factor in the feedback loop (applied identically on all backends, not WebGPU-specific — would need to become a shared-code change, not a `drivers/webgpu/`-only one) or confirming some other discrepancy between the two code paths beyond storage precision. **Next step for whoever picks this up**: instrument (temporary `print_line`/readback) the actual per-frame magnitude of `light_total`/`irradiance` at a fixed probe location across both backends to see whether Vulkan's packed path is genuinely converging to a bounded value (supporting the damping theory) or clipping/staying flat for a different reason entirely.
...
**Suggested next round priorities, in order**: (1) item 11 — instrument actual per-frame `light_total`/`irradiance` magnitude on both backends to test the "packed-format quantization provides implicit damping" hypothesis before attempting any further fix; this is now a shared-code (not `drivers/webgpu/`-only) question if confirmed, (2) get the user's own real hands-on confirmation that SDFGI's *rendering* (independent of the brightness bug) and SSAO's fix both hold on their actual machine (Round 27/28/30's verification are all sandbox-only), (3) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite — and while doing so, audit/fix the `extensions_support` export-preset gap Round 23 found so future scratch projects don't silently test a stale template, (4) item 9 (canvas preferred-format warning) as time allows.
