...
**Current standing (2026-09-10, end of this session)**: Tasks 9.1–9.3 are done at the shader-conversion level. Task 9.5 has now fixed **fourteen** real, live-verified bugs across six rounds. Two of Round 5's four fixes needed no changes (attachment-format fix, device limits) and are confirmed solid against fresh real hardware; a third (standalone depth-aspect view) is also confirmed solid; the fourth (VoxelGi depth-reclassification false positive) had the right diagnosis of the *symptom* but the wrong mechanism, now corrected and verified. Round 6 also fixed the single most frequent remaining error shape (the 3D/2D view-dimension mismatch, 81 occurrences in the prior log). Remaining **not yet root-caused**, all lower-frequency and non-cascading: `SdfgiIntegrateShaderRD:0`'s Sint/UnfilterableFloat BGL mismatch at binding 19 (investigated at length in Round 5 without success — worth revisiting with the same "temporary print + local live-repro" technique that finally cracked the VoxelGi case this round, rather than more standalone `glslangValidator` reconstruction, which has now twice failed to reproduce the real engine's exact binding layout); `ResolveShaderRD`'s sampler/texture type mismatch; `SsaoInterleaveShaderRD`'s storage-format mismatch. `screen_space_reflection_filter.glsl`'s formatless storage image (Task 8.3) remains identified-but-unfixed and unrelated to this work. The next step is another fresh real-hardware log to confirm this round's two fixes and continue down the shrinking remaining-issues list; the total unique error count has gone from ~19-20 (Round 4's end) to 15 (Round 5) and should drop further once Round 6's fixes are confirmed. Real hardware data (Chrome 48-texture/16-sampler, Firefox flat-64, from Round 4) still stands and is worth weighing against continued investment here versus this fork's existing, hardened Mobile-renderer focus.
