...
## CURRENT STATUS (updated after Round 26, 2026-09-11) — read this first
...
1. **[LOW, found in Round 24, re-identified in Round 25, now the sole documented blocker keeping SDFGI from rendering, not yet fixed] `GiShaderRD:2:set0` binding 2 (`sdf_cascades[]`): `Dimension (TextureViewDimension::e2D) of [TextureView of Texture (unlabeled 4x4 px, TextureFormat::RGBA8Unorm)] doesn't match the expected dimension (TextureViewDimension::e3D)`.** Was a single hit per capture while item 2 below (now fixed) dominated the log; with that noise gone, this is now the **dominant** error source (~10,700 total errors per 65s capture, almost all cascading `"is invalid due to a previous error"` spam from this one root cause). Probably fires for one unused SDFGI cascade slot (`sdf_cascades[SDFGI_MAX_CASCADES]`, a `texture3D` array in `gi.glsl`, binding 1 → WGSL binding 2) when fewer cascades are configured than `SDFGI_MAX_CASCADES`. **Strong lead from Round 25, still unchecked**: the 4×4 `RGBA8Unorm` placeholder texture is the exact same shape as Round 25's `voxel_gi_buffer` bug (a wrong-typed default-texture fallback) — check `gi.cpp`'s unused-cascade-slot fallback for this binding first, before assuming a new bug class; it may just need a 3D-shaped default (`DEFAULT_RD_TEXTURE_3D_WHITE`/`_BLACK`, already used correctly elsewhere in this same file for sibling cascade-array bindings) instead of whatever 2D default is landing here.
...
**Suggested next round priorities, in order**: (1) item 1 — the `GiShaderRD` `sdf_cascades[]` Dimension mismatch, now the sole documented blocker keeping SDFGI from rendering and the dominant error source by far; worth checking against Round 25's "wrong-typed default fallback" bug shape first (a much smaller fix than a new investigation if it's the same pattern, and Round 26's own experience suggests it's worth budgeting time for a follow-on pipeline-pre-warming-style gap even if the primary fix looks simple), (2) once item 1 lands, another fresh full capture to check what's left (same pattern every round since 19 has followed) — this may finally be the round SDFGI actually renders, (3) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite — and while doing so, audit/fix the `extensions_support` export-preset gap Round 23 found so future scratch projects don't silently test a stale template, (4) items 8-9 (SsaoInterleave R8Unorm, canvas preferred-format warning) as time allows.
