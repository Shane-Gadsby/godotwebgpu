...
**Current standing (2026-09-10, end of this session)**: Tasks 9.1–9.3 are done at the shader-conversion level. Task 9.5 has now made **twenty** fix attempts across eleven rounds; nineteen are live-verified as genuinely fixing their target error, with Round 11 also retroactively confirming Round 8's fix was directionally correct (same bug class) even though it wasn't the actual trigger for the observed error. Every error present at the start of this Forward+ live-repro effort has now been found and fixed, with only Task 8.3 remaining as a separate, pre-existing, out-of-scope gap. The next step is one more real-hardware log to do a final overall confirmation pass — at this point mostly a formality given how directly Round 11's fix was verified — after which the natural question becomes whether to invest in Task 8.3 next or consider this Forward+ effort's live-repro phase complete and weigh further investment here against this fork's existing, hardened Mobile-renderer focus. Real hardware data (Chrome 48-texture/16-sampler, Firefox flat-64, from Round 4) still stands and remains relevant to that decision regardless.
