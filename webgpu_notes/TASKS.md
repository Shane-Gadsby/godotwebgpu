...
**Current standing (2026-09-10, end of this session)**: Tasks 9.1–9.3 are done at the shader-conversion level. Task 9.5 has now made **twenty-one** fix attempts across twelve rounds; twenty are live-verified as genuinely fixing their target error. Round 12's fix addresses what may be the most impactful bug found this entire session, given cluster building runs on every single Forward+ frame regardless of scene content — unlike every other fix, which was scoped to a specific shader/feature. `screen_space_reflection_filter.glsl`'s formatless storage image (Task 8.3) remains the one identified-but-unfixed, separately-scoped gap. The next step is a fresh log from the user's real project to confirm Round 12's fix holds outside this sandbox and see what (if anything) remains — this is the first round all session verified only in-sandbox against a fix targeting a real-project-only symptom, so real-hardware confirmation matters more here than for most of the preceding rounds.
