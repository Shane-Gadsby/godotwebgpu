...
**Current standing (2026-09-10, end of this session)**: Tasks 9.1–9.3 are done at the shader-conversion level. Task 9.5 has now made **sixteen** fix attempts across eight rounds, fifteen of which are live-verified and one (this round's) structurally sound but not yet confirmed against real hardware. Remaining **not yet root-caused**, both lower-frequency and non-cascading: `ResolveShaderRD`'s sampler/texture type mismatch; `SsaoInterleaveShaderRD`'s storage-format mismatch (found during this round's investigation that the BGL label on at least one of these errors was shared between two structurally-identical-but-different shaders — `bgl:SsaoInterleaveShaderRD:0:set0` validated against an `OctmapRoughnessShaderRD` module — almost certainly Dawn's own BindGroupLayoutDescriptor deduplication reusing the first-created object's debug label, not a sign that these two shaders are actually related; don't trust these labels at face value when investigating). `screen_space_reflection_filter.glsl`'s formatless storage image (Task 8.3) remains identified-but-unfixed and unrelated to this work. The next step is another fresh real-hardware log to (a) confirm whether this round's `texture_create_shared_from_slice` fix actually eliminates the remaining dimension-mismatch occurrences, and (b) continue down the Resolve/SsaoInterleave items with the same live-repro approach, ideally starting from a temporary diagnostic print rather than static analysis given how unreliable static reconstruction of the real engine's exact binding layout has proven across Rounds 5-7. Real hardware data (Chrome 48-texture/16-sampler, Firefox flat-64, from Round 4) still stands and is worth weighing against continued investment here versus this fork's existing, hardened Mobile-renderer focus.
