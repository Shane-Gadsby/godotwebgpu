...
## CURRENT STATUS (updated after Round 20, 2026-09-11) — read this first
...
1. **[HIGH, architectural — root cause of the SDFGI black screen, found in Round 20] SDFGI's packed-format texture-reinterpretation trick (Godot's `shareable_formats`/`format_override` mechanism) has no working equivalent on WebGPU, and the resulting `wgpuTextureCreateView` validation failures poison every subsequent frame's command buffer for the rest of the session.** See Round 20 below for the full trace. Three real textures use this trick (`gi.cpp`): `lightprobe_data` (R32Uint, GLSL binding for octahedral probes) viewed as `lightprobe_texture` (E5B9G9R9_UFLOAT_PACK32/RGB9E5Ufloat), `cascade.light_data` (R32Uint per-cascade 3D light-packing) viewed as `cascade.light_tex` (same RGB9E5Ufloat reinterpretation), and `occlusion_data` (R16Uint) viewed as `occlusion_texture` (R4G4B4A4_UNORM_PACK16 — which is *also* entirely unmapped in `_data_format_to_wgpu()`, a second independent problem on top of the first). Root cause: WebGPU requires a texture view's format to either match the source texture's format or appear in its declared `viewFormats` list (which this driver never populates from Godot's `shareable_formats` beyond the automatic sRGB pair — see Round 19's key-doubling note for the sibling mechanism), **and**, more fundamentally, RGB9E5Ufloat is not a storage-capable WebGPU format at all, so *any* view request for it on a texture carrying `StorageBinding` usage is rejected outright regardless of `viewFormats` — confirmed via the live error: `The texture usage (...StorageBinding...) includes TextureUsage::StorageBinding, which is incompatible with the format (TextureFormat::RGB9E5Ufloat)`, thrown from `texture_create_shared()`'s `wgpuTextureCreateView` call. This driver's vendored `webgpu.h` (`emsdk/.../system/include/webgpu/webgpu.h`) has no `usage` field on `WGPUTextureViewDescriptor` either, so there's no way to request a *narrower*-usage view to route around it the way `_is_srgb_format()`'s existing storage/sRGB workaround does (that one just falls back to the *same* format instead, which isn't an option here since the whole point is reading it in the packed format). **Real fix needs architectural work, not a quick patch**: either (a) give these three logical textures two *separate* real `WGPUTexture` backing objects — one StorageBinding-capable in the raw integer format for the write passes, one TextureBinding-only in a real sampleable float format — with an explicit decode compute/copy pass bridging them (RGB9E5Ufloat itself still isn't an option as the second texture's format since it can't be storage-written either; would need e.g. RGBA16Float and a shader-side format change), or (b) change `sdfgi_direct_light.glsl`/`sdfgi_integrate.glsl`/`gi.glsl` to stop RGBE/packed-uint encoding for these three textures entirely and just declare them RGBA16Float (or another WebGPU-native dual-purpose format) from the start, updating the packing/unpacking GLSL math accordingly. Both are real shader/engine-side changes, not driver-only — not attempted this round given the risk of subtly breaking SDFGI's HDR probe-irradiance precision without careful verification. **Not urgent to re-verify with a short capture** — expect this to still reproduce until one of the above is implemented.

2. **[MEDIUM, found in Round 20] Round 19's pre-DCE storage-image scan (`_extract_pre_dce_storage_image_info()`) has at least one remaining gap: it still misses `sdfgi_direct_light.glsl`'s `dst_light` (`r32ui`, GLSL binding 6 → WGSL binding 12) and at least one `SdfgiIntegrateShaderRD` binding (WGSL binding 18, real format RGBA16Sint/30-layer, likely `lightprobe_history_scroll`), both of which still fall back to the RGBA8Unorm default and fail bind-group creation** (`Format (TextureFormat::R32Uint) ... expected to be (TextureFormat::RGBA8Unorm)`, and the RGBA16Sint equivalent). Unlike the SSAO/SSIL case Round 19 fixed, `dst_light` is a plain, non-array, non-combined `uimage3D` declaration — structurally identical to what the fix targets — so the gap's cause isn't obvious from inspection alone; needs an actual instrumented build (a debug print inside `_extract_pre_dce_storage_image_info()`'s per-variable loop, keyed by shader name) to see whether the SPIR-V walk is finding and mis-keying this variable, or never finding it at all. Possible leads not yet chased: whether `SdfgiDirectLightShaderRD`/`SdfgiIntegrateShaderRD` go through the legacy spec-constant re-conversion path (`_create_module_with_spec_constants()`, which never got Round 19's pre-DCE seeding call at all — see that function's call sites, none adjacent to `_extract_pre_dce_storage_image_info()`) for some variants but not others.

3. **[MEDIUM, found in Round 20] `SceneForwardClusteredShaderRD`'s fragment stage (legacy spec-constant path, `specmod#49`), binding 54: `The layout's binding access (StorageTextureAccess::WriteOnly) isn't compatible with the shader's binding access (StorageTextureAccess::ReadWrite)`.** Same bug *shape* as Round 16's DOF read_write-storage-shadow-refresh fix, but for whichever SDFGI-related texture (likely `lightprobe_texture` or `ambient_texture`, used read_write for self-illumination feedback during compositing) is bound at set1/binding54 in this specialized module. Not yet identified precisely or fixed.

4. **[MEDIUM, found in Round 20] `SdfgiPreprocessShaderRD:7:set0`: `Texture view array layer range (baseArrayLayer: 0, arrayLayerCount: 30) exceeds the texture's array layer count (1)`** on a 289×272 RGBA16Sint texture — almost certainly `lightprobe_history_scroll` (`tf_probe_history`, format `R16G16B16A16_SINT`, `array_layers = history_size`; 289×272 matches `probe_axis_count`-derived dimensions). The *view* correctly requests all `history_size` (30) layers, but the *real* underlying `WGPUTexture` this driver created for it apparently only has 1 array layer — a `texture_create()`-side construction bug (array_layers not being read/passed correctly for this texture, or a `TEXTURE_TYPE_2D_ARRAY` vs `TEXTURE_TYPE_2D` mixup), not a view-creation bug. Not yet chased.
...
7. **[VERY LOW, cosmetic — subsumed by item 1] One-time `WebGPU: Unsupported DataFormat 1` warning** (`DATA_FORMAT_R4G4B4A4_UNORM_PACK16`, fires once during SDFGI startup, self-suppresses on repeat). `_data_format_to_wgpu()` has no mapping for this obscure packed format. Round 20 traced this to `occlusion_texture`'s `format_override` (see item 1) — not independently fixable via a format-table addition, since WebGPU has no native RGBA4-style format at all; resolved only as a side effect of item 1's eventual real fix.

**Suggested next round priorities, in order**: (1) decide on and implement a real fix strategy for item 1 (the architectural SDFGI packed-format gap — this is what's actually blocking the user's current real settings; needs a design decision, see the two options above, before more code is written), (2) item 2 (finish closing Round 19's pre-DCE scan gap — smaller, more self-contained, worth doing even before item 1 lands since it likely affects other not-yet-tested shaders too), (3) items 3-4 (SDFGI-specific but more self-contained than item 1), (4) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite, (5) items 5-7 as time allows.
