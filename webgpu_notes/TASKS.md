...
## CURRENT STATUS (updated after Round 23, 2026-09-11) — read this first
...
**Suggested next round priorities, in order**: (1) item 1 — the `SdfgiIntegrateShaderRD` BGL entry-count mismatch, now the sole known blocker for SDFGI actually rendering; instrument the merged-entries construction path per item 1's own "Next step" note, (2) once item 1 lands, a fresh full capture to check whether anything else was being masked by it (same pattern as Round 22's and Round 23's own discoveries), (3) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite — and while doing so, audit/fix the `extensions_support` export-preset gap Round 23 found so future scratch projects don't silently test a stale template, (4) items 5-6 as time allows.
