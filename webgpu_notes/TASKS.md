...
11. **[HIGH, new — found 2026-09-11, NOT ROOT-CAUSED, top priority] SDFGI's brightness/exposure climbs continuously over time with SDFGI enabled, until the 3D scene goes solid white — with zero console errors.** Reported directly by the user on their real machine, immediately after Round 27/28's milestone. Resets back to normal whenever any SDFGI field is changed (expected: a field change reallocates `gi.cpp`'s SDFGI cascades and clears the history/light textures — see `SDFGI::create()`), then climbs toward white again. This is a pure visual/algorithmic runaway, not a validation error — Round 29's partial investigation (interrupted by a session rate limit before reaching a conclusion) confirmed **zero console errors** during the drift, matching the user's report, via a fresh capture against a scratch copy of the real project (script left behind: `webgpu_tests/screenshot_comparison/probe_brightness_trend_r29.mjs`, untracked). Round 29 did not get far enough to determine scope before being cut off — this is the critical open question and the mandatory first step for whoever picks this up: **does the same drift happen on the native Vulkan editor build, using the identical scratch project/settings, over the same multi-minute duration?** If yes: this is a pre-existing upstream Godot SDFGI algorithm/tuning behavior (plausible on its own merits — SDFGI has never rendered successfully for more than a few seconds anywhere in this task's 27-round history, on any backend, so a slow energy-feedback issue could easily have gone unnoticed until now) and is out of scope for `drivers/webgpu/` — document and suggest a project-side mitigation (e.g. check the `Environment` SDFGI `energy` setting) rather than modifying shared SDFGI algorithm code. If no (Vulkan stays stable, WebGPU alone drifts): this is a real bug specific to Round 21's WebGPU-only storage-format substitution, and the most promising lead going in is a **missing clamp on the native-storage-format write path**: the original RGBE9995-packed path clamps every value to a fixed representable ceiling before packing (`clamp(light_total.r, 0.0, 65408.0)` in `sdfgi_direct_light.glsl`'s non-native branch), but the `#ifdef SDFGI_NATIVE_STORAGE_FORMAT` branches in both `sdfgi_direct_light.glsl` (`imageStore(dst_light, positioni, vec4(light_total, 0.0))`) and `sdfgi_integrate.glsl` (`imageStore(lightprobe_texture_data, copy_to[i], vec4(irradiance, 0.0))`) write the raw float with **no equivalent bound** — if that implicit ceiling was accidentally load-bearing for the whole feedback loop's numerical stability (self-illumination bouncing back through cascades frame over frame), removing it on WebGPU only could be exactly why energy now accumulates unbounded there while staying self-limiting on the packed-format backends. Not yet verified either way — this is a hypothesis to test, not a confirmed diagnosis. Corroborating evidence already in this doc: Round 27's own SDFGI+Glow+Fog+Adjustment combined-test screenshot was noted as "heavily over-exposed but non-erroring," at the time guessed to be "a legitimate scene/tonemap interaction" — very likely this same bug, unrecognized at the time.

**Suggested next round priorities, in order**: (1) item 11 — root-cause and (if in scope) fix the SDFGI brightness-runaway bug; start with the Vulkan-vs-WebGPU comparison to determine scope before writing any code, (2) get the user's own real hands-on confirmation that SDFGI's *rendering* (independent of the brightness bug) and SSAO's fix both hold on their actual machine (both Round 27's and Round 28's verification are sandbox-only), (3) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite — and while doing so, audit/fix the `extensions_support` export-preset gap Round 23 found so future scratch projects don't silently test a stale template, (4) item 9 (canvas preferred-format warning) as time allows.
