...
## CURRENT STATUS (updated after Round 22, 2026-09-11) — read this first
...
**Suggested next round priorities, in order**: (1) item 1 — the bind-group/BGL construction gap for specialization-discovered read_write-storage splits (the last known blocker for SDFGI actually rendering; needs to extend whatever builds bind-group entries from `shader->rw_storage_splits` to also handle a split discovered only in a specialized module, plus fix the shadow sample-type-format lookup gap the original split code already flags), (2) once item 1 lands, a fresh full capture to check whether anything else was being masked by it (same pattern as this round's own item 1/2 discovery), (3) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite, (4) items 4-5 as time allows.
