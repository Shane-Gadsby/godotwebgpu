...
8. **[LOW — doesn't reproduce under the user's actual per-feature settings] `SsaoInterleaveShaderRD`'s `dest_image` binding (set0/binding0) sometimes gets a real `R8Unorm` texture where the shader layout correctly expects `RGBA8Unorm`.** First seen in Round 17 (`ssr`+`ssao`+`ssil` enabled, `sdfgi` disabled), re-confirmed independently in Round 19 while stress-testing all seven non-SDFGI effects enabled simultaneously. Confirmed in Round 19 that this is **not** a DCE/WGSL-text problem (the binding is written unconditionally in every shader variant, so the existing text scan always finds `rgba8` correctly) and **not** caused by either of Round 19's fixes (verified: the new pre-DCE seeding map is function-local per shader-container call and only ever inserts-if-absent, never overwrites). The bug is therefore on the **engine/`ss_effects.cpp` side** — the wrong real texture is being selected/bound for some `downsample_mode` combination. **Next step**: bind-group-entry-level instrumentation — log the actual `WGTexture*`/format at the exact `UNIFORM_TYPE_IMAGE` bind-group-entry construction site in `uniform_set_create()` when this specific mismatch fires, cross-reference against `ss_effects.cpp`'s texture-selection logic for whichever `downsample_mode` is active in the repro. Not urgent: doesn't reproduce with one-effect-at-a-time testing, which is how the user actually configures their project today.
...
**Suggested next round priorities, in order**: (1) get the user's own real hands-on confirmation that SDFGI now renders correctly on their actual machine/project (this round's verification is sandbox-only, per the "Confirmed working" line above) — this is the one thing standing between "sandbox-verified" and actually closing out Task 9.5's core goal, (2) item 8 — the pre-existing `SsaoInterleaveShaderRD` R8Unorm-vs-RGBA8Unorm bug, now the only known issue when multiple effects are stacked together (confirmed independent of SDFGI in Round 27's own combined test) — still not urgent since it doesn't reproduce under one-effect-at-a-time settings, but worth picking up if the user starts combining effects, (3) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite — and while doing so, audit/fix the `extensions_support` export-preset gap Round 23 found so future scratch projects don't silently test a stale template, (4) item 9 (canvas preferred-format warning) as time allows.
