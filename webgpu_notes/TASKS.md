...
**Suggested next round priorities, in order**: (1) item 11 — instrument `cascade.light_data` (direct_light's per-voxel output) immediately after `update_light()`'s dispatch on both backends, the same way Round 31 instrumented `lightprobe_data`, to confirm whether the divergence already exists at that earlier stage (would confirm `sdfgi_direct_light.glsl`'s bounce-feedback read/write as the exact site) before attempting any fix, (2) get the user's own real hands-on confirmation that SDFGI's *rendering* (independent of the brightness bug) and SSAO's fix both hold on their actual machine (Round 27/28/30/31's verification are all sandbox-only), (3) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite — and while doing so, audit/fix the `extensions_support` export-preset gap Round 23 found so future scratch projects don't silently test a stale template, (4) item 9 (canvas preferred-format warning) as time allows.
