...
## CURRENT STATUS (updated after Round 27, 2026-09-11) — read this first

**Milestone: SDFGI genuinely renders end-to-end on WebGPU as of Round 27** — the first time in this task's history. A fresh 65s capture against the user's real project (SDFGI-only) shows **zero console errors** and a real, lit scene (not a solid-black viewport) — confirmed via screenshot. A broader sanity pass (SDFGI + Glow + Fog + Adjustment stacked) also came back clean, zero errors. The one caveat found along the way: stacking SDFGI with `ssr`+`ssao`+`glow` reproduces the already-documented, pre-existing `SsaoInterleaveShaderRD` bug (item 8 below) — confirmed independent of SDFGI and not a regression from this round's fix.
...
8. **[HIGH — corrected 2026-09-11, reproduces on the user's real machine with SSAO alone] `SsaoInterleaveShaderRD`'s `dest_image` binding (set0/binding0) sometimes gets a real `R8Unorm` texture where the shader layout correctly expects `RGBA8Unorm`, spamming the console.** First seen in Round 17 (`ssr`+`ssao`+`ssil` enabled, `sdfgi` disabled), re-confirmed independently in Round 19 while stress-testing all seven non-SDFGI effects enabled simultaneously, and at the time believed **not** to reproduce with any single effect enabled alone. **That belief was wrong**: the user reports (and the repo's own `errors-chrome.txt` scratch capture confirms — `grep -n "SsaoInterleaveShaderRD" errors-chrome.txt` shows the exact `Format (TextureFormat::R8Unorm) ... expected to be (TextureFormat::RGBA8Unorm)` / `bgl:SsaoInterleaveShaderRD:1:set0` signature) that enabling **SSAO by itself** reproduces this on their real hardware, spamming the console continuously — this was never actually tested in isolation on real hardware before now, only in sandbox one-effect-at-a-time sweeps that apparently didn't hit whichever `ss_effects.cpp` `downsample_mode`/quality path the user's real settings select. Confirmed in Round 19 that this is **not** a DCE/WGSL-text problem (the binding is written unconditionally in every shader variant, so the existing text scan always finds `rgba8` correctly) and **not** caused by either of Round 19's fixes (verified: the new pre-DCE seeding map is function-local per shader-container call and only ever inserts-if-absent, never overwrites). The bug is therefore on the **engine/`ss_effects.cpp` side** — the wrong real texture is being selected/bound for some `downsample_mode`/quality combination. **Next step**: bind-group-entry-level instrumentation — log the actual `WGTexture*`/format at the exact `UNIFORM_TYPE_IMAGE` bind-group-entry construction site in `uniform_set_create()` when this specific mismatch fires, cross-reference against `ss_effects.cpp`'s texture-selection logic for whichever `downsample_mode`/quality level SSAO-alone actually selects. **Now the top priority — it's the only known remaining defect that reproduces under the user's actual, real, single-effect settings**, unlike every other item on this list.
...
**Suggested next round priorities, in order**: (1) item 8 — the `SsaoInterleaveShaderRD` R8Unorm-vs-RGBA8Unorm bug, corrected 2026-09-11 to **HIGH** priority now that the user's real hardware confirms it reproduces with SSAO enabled alone (not just multi-effect stacking as previously believed) — this is now the only known defect hitting the user's actual real settings, (2) get the user's own real hands-on confirmation that SDFGI now renders correctly on their actual machine/project (Round 27's verification is sandbox-only, per the "Confirmed working" line above) — this is the one thing standing between "sandbox-verified" and actually closing out Task 9.5's core goal, (3) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite — and while doing so, audit/fix the `extensions_support` export-preset gap Round 23 found so future scratch projects don't silently test a stale template, (4) item 9 (canvas preferred-format warning) as time allows.
