...
**Current standing (2026-09-10, end of this session)**: Tasks 9.1–9.3 are done at the shader-conversion level. Task 9.5 has now made **eighteen** fix attempts across nine rounds; seventeen are live-verified (sixteen genuinely fixed something real, one — Round 8's — verified only as "doesn't break anything," not as fixing the target error it was written for). The three items flagged at the end of Round 8 are now: two fixed and verified (Resolve, SsaoInterleave/Octmap), one still open and now confirmed *not* explained by Round 8's fix (the 3D/2D dimension error, 72 stable occurrences, cause unknown). `screen_space_reflection_filter.glsl`'s formatless storage image (Task 8.3) remains identified-but-unfixed and unrelated to this work. This round's fixes are **not yet verified against a fresh real-hardware log** — next step is getting one to confirm the Resolve and Octmap fixes hold, and to get updated data on the one remaining dimension-error mystery. Real hardware data (Chrome 48-texture/16-sampler, Firefox flat-64, from Round 4) still stands and is worth weighing against continued investment here versus this fork's existing, hardened Mobile-renderer focus — though with the error list now down to essentially one confirmed-real remaining item plus the long-known Task 8.3 gap, this Forward+ effort is close to as clean as this driver's live-repro methodology alone can likely get it without either real-hardware-side tooling or a different diagnostic vantage point.
