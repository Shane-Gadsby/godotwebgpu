...
## CURRENT STATUS (updated after Round 21, 2026-09-11) — read this first
...
1. **[MEDIUM, found in Round 20, still open] Round 19's pre-DCE storage-image scan (`_extract_pre_dce_storage_image_info()`) has at least one remaining gap: it still misses `sdfgi_direct_light.glsl`'s `dst_light` (`r32ui`, GLSL binding 6 → WGSL binding 12) and at least one `SdfgiIntegrateShaderRD` binding (WGSL binding 18, real format RGBA16Sint/30-layer, `lightprobe_history_scroll` — see item 2's array-layer-count confirmation), both of which still fall back to the RGBA8Unorm default and fail bind-group creation** (`Format (TextureFormat::R32Uint) ... expected to be (TextureFormat::RGBA8Unorm)`, and the RGBA16Sint equivalent). Unlike the SSAO/SSIL case Round 19 fixed, `dst_light` is a plain, non-array, non-combined `uimage3D` declaration — structurally identical to what the fix targets — so the gap's cause isn't obvious from inspection alone; needs an actual instrumented build (a debug print inside `_extract_pre_dce_storage_image_info()`'s per-variable loop, keyed by shader name) to see whether the SPIR-V walk is finding and mis-keying this variable, or never finding it at all. Possible leads not yet chased: whether `SdfgiDirectLightShaderRD`/`SdfgiIntegrateShaderRD` go through the legacy spec-constant re-conversion path (`_create_module_with_spec_constants()`, which never got Round 19's pre-DCE seeding call at all) for some variants but not others. **Now the actual next thing keeping SDFGI from rendering, per Round 21 (see below) — item 4 (formerly item 1, the packed-format-reinterpretation root cause) is fixed.**

2. **[MEDIUM, found in Round 20, still open] `SdfgiPreprocessShaderRD:7:set0`: `Texture view array layer range (baseArrayLayer: 0, arrayLayerCount: 30) exceeds the texture's array layer count (1)`** on a 289×272 RGBA16Sint texture — confirmed by Round 21's error log to be `lightprobe_history_scroll` (`tf_probe_history`, format `R16G16B16A16_SINT`, `array_layers = history_size`). The *view* correctly requests all `history_size` (30) layers, but the *real* underlying `WGPUTexture` this driver created for it apparently only has 1 array layer — a `texture_create()`-side construction bug (array_layers not being read/passed correctly for this texture, or a `TEXTURE_TYPE_2D_ARRAY` vs `TEXTURE_TYPE_2D` mixup), not a view-creation bug. Not yet chased.

3. **[MEDIUM, found in Round 20, still open] `SceneForwardClusteredShaderRD`'s fragment stage (legacy spec-constant path, `specmod#49`), binding 54: `The layout's binding access (StorageTextureAccess::WriteOnly) isn't compatible with the shader's binding access (StorageTextureAccess::ReadWrite)`.** Same bug *shape* as Round 16's DOF read_write-storage-shadow-refresh fix, but for whichever SDFGI-related texture (likely `lightprobe_texture` or `ambient_texture`, used read_write for self-illumination feedback during compositing) is bound at set1/binding54 in this specialized module. Not yet identified precisely or fixed.

4. **[FIXED in Round 21, verified] SDFGI's packed-format texture-reinterpretation trick (Godot's `shareable_formats`/`format_override` mechanism) had no working equivalent on WebGPU — this was the actual root cause of the original "SDFGI black screen" mystery (Rounds 17-20).** Fixed via a new `RD::SUPPORTS_SHAREABLE_TEXTURE_FORMATS` driver trait (true on Vulkan/Metal/D3D12, false on WebGPU) that gates `gi.cpp`'s three affected textures (`lightprobe_data`/`lightprobe_texture`, `cascade.light_data`/`light_tex`, `occlusion_data`/`occlusion_texture`) between the original packed-uint-plus-aliased-view approach and, on WebGPU, storing already-decoded values directly in a natively-supported format (`RGBA16_SFLOAT` for light/lightprobe, `RGBA8_UNORM` for occlusion) with no aliasing at all. See Round 21 below for the full implementation and verification trail, and for why this is a *smaller* change than it sounds (every read site was already a plain float sample, needing zero changes). **The scene still does not visibly render on WebGPU** — items 1-3 above are separate, already-catalogued bugs that this fix's absence had been masking (the shareable-format errors fired early enough in startup to poison every subsequent frame before these three ever got a chance to manifest); Round 21's own error-log diff confirms items 1-3 are unaffected by (not caused by, not fixed by) this change.
...
7. **[RESOLVED as a side effect of item 4] One-time `WebGPU: Unsupported DataFormat 1` warning** (`DATA_FORMAT_R4G4B4A4_UNORM_PACK16`, fired once during SDFGI startup). `_data_format_to_wgpu()` has no mapping for this obscure packed format — no longer reachable on WebGPU now that `occlusion_texture` no longer requests it (item 4's fix). Not independently re-verified as gone from a fresh capture, but no code path can reach it anymore.

**Suggested next round priorities, in order**: (1) item 1 (finish closing Round 19's pre-DCE scan gap — now the most likely thing directly blocking SDFGI's visible render, and probably affects other not-yet-tested shaders too), (2) item 2 (`lightprobe_history_scroll`'s array-layer-count bug — self-contained, likely a one-line `texture_create()` fix once located), (3) item 3, (4) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite, (5) items 5-6 as time allows.
