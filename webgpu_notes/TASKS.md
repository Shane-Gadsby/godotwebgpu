...
## CURRENT STATUS (updated after Round 28, 2026-09-11) — read this first

**Milestone: SDFGI genuinely renders end-to-end on WebGPU as of Round 27, and the last known defect reproducing under the user's real, single-effect settings (`SsaoInterleaveShaderRD`, item 8) is fixed as of Round 28.** SDFGI: a fresh 65s capture against the user's real project (SDFGI-only) shows **zero console errors** and a real, lit scene (not a solid-black viewport) — confirmed via screenshot; sandbox-verified only, not yet confirmed by the user's own hands-on testing (see item 2 of "Suggested next round priorities"). SSAO: a genuine, pre-existing upstream Godot format inconsistency (a storage image's declared `rgba8` format never matched its real `R8_UNORM` backing texture, tolerated by Vulkan/Metal/D3D12's looser validation but rejected by WebGPU) is fixed — verified with SSAO enabled alone (the user's actual repro, never previously tested standalone) and with SSAO+SSR+Glow+SDFGI all stacked, both zero errors.
...
**Suggested next round priorities, in order**: (1) get the user's own real hands-on confirmation that SDFGI now renders correctly on their actual machine/project, and that SSAO's console spam is gone (both Round 27's and Round 28's verification are sandbox-only) — this is the one thing standing between "sandbox-verified" and actually closing out Task 9.5's core goal, since no other known defect currently hits the user's real settings, (2) decide whether to formalize the Rounds 15-19 scratch-copy-and-probe workflow into a proper `webgpu_tests/` regression suite — and while doing so, audit/fix the `extensions_support` export-preset gap Round 23 found so future scratch projects don't silently test a stale template, (3) item 9 (canvas preferred-format warning) as time allows.
