...
**Current standing (2026-09-10, end of this session)**: Tasks 9.1–9.3 are done at the shader-conversion level. Task 9.5 has now made **nineteen** fix attempts across ten rounds; eighteen are live-verified as genuinely fixing their target error (Round 8's remains the sole exception — verified only as "doesn't break anything"). Of the errors present at the start of this whole Forward+ live-repro effort, only two items remain unaddressed: the 3D/2D texture-view dimension error (cause still unknown, not reproducible in this sandbox) and `screen_space_reflection_filter.glsl`'s formatless storage image (Task 8.3, a separate, larger, pre-existing gap needing real runtime-format injection). Every other real bug surfaced by five consecutive real-hardware runs (Rounds 5 through 10) has been found and fixed. The natural next step is one more real-hardware log to see whether the 3D/2D error is the only thing left standing between visually-broken and visually-correct output, and if so, whether it's worth the investment of real-hardware-side tooling (e.g., a small standalone page in the user's own browser that monkey-patches `GPUDevice.createTexture`/`createView` to log real call arguments) to finally identify it. Real hardware data (Chrome 48-texture/16-sampler, Firefox flat-64, from Round 4) still stands and remains worth weighing against continued investment here versus this fork's existing, hardened Mobile-renderer focus.
