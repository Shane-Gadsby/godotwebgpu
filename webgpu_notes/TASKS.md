...
**Coverage gap worth noting**: neither `skeleton.glsl` nor `particles_copy.glsl` (nor `canvas.glsl`) appear in `wgsl_precompile.py`'s `SHADER_REGISTRY` — it's a curated, hand-maintained list, not a directory scan, so bugs in shaders missing from it are invisible to build-time precompile testing and only surface at runtime, as they did here. Worth auditing `SHADER_REGISTRY` against the full `servers/rendering/renderer_rd/shaders/` tree for other gaps; not done in this session.
