...
## 10. Follow-up investigation (2026-09-20, branch `webgpu-4.7.2`)

Revisited while investigating "loading progress bar hits 100% long before the game is playable" (user-reported). Two findings that change the risk picture for this doc's plan:

### 10.1 The `async_shader_pipeline` branch no longer exists

Neither the branch nor commits `864e936cd3`/`eb9d522e92` exist in this fork's local repo or its `origin`/`upstream` remotes (`Shane-Gadsby/godotwebgpu` and `godotengine/godot`). They were apparently local-only on the original author's (`dwalter`) machine and never pushed here. **The Tint Web Worker code described in Section "What Was Built" above cannot be cherry-picked — it would need to be rebuilt from scratch** following this doc's design, including a new Emscripten build target for a standalone Tint-conversion WASM module the worker can load (the main engine's Tint is linked directly into `godot.wasm`, not a separate loadable module — see CLAUDE.md's shader pipeline diagram).

Current-code equivalents of the line references above (branch `webgpu-4.7.2`):
- `_spv_to_wgsl_cached()`: `drivers/webgpu/rendering_device_driver_webgpu.cpp:180-243` (3-tier lookup: in-memory cache → build-time precompiled table `wgsl_precompiled.gen.h` → Tint fallback). Cache is now a 64-bit-keyed `HashMap<uint64_t, String>` (two MurmurHash3 passes), not whatever key size existed at 4.6.2.
- Sync Tint fallback call sites: `shader_create_from_container()` (starts line 4597, calls `_spv_to_wgsl_cached()` at 4771) and `_create_module_with_spec_constants()` (starts line 9774, call at 9782) — the latter is the one that matters for this doc's plan, since it's specialization-constant-patched (per-material, per-runtime-state) shaders that can't be baked ahead of time.
- `tint_wrapper_spirv_to_wgsl()` (`drivers/webgpu/tint_wrapper.cpp:45-130`) is confirmed stateless/pure w.r.t. engine globals — no references to `_spv_to_wgsl_cache` or other engine state inside it, only Tint's own internal state (`tint::Initialize()` — whether that's safe to have a second independent copy of in a Worker's own WASM instance needs checking against Tint's own docs, not evaluated here). This part of the plan is still sound.
- No generic Web Worker infrastructure exists anywhere in `platform/web/js/` (only `AudioWorkletProcessor`-based worklets, a different API shape) — a Tint worker would be net-new JS, not an extension of an existing pattern.

### 10.2 A harder blocker than the emdawnwebgpu callback bug: `PipelineHashMapRD` has no retry state

This doc's plan assumed Part 1 (Tint Worker) alone was mergeable independently of Part 2 (async GPU pipeline creation), on the theory that deferring a Tint conversion just means `_create_module_with_spec_constants()` returns nullptr for one frame and the existing ubershader-fallback mechanism (`render_forward_mobile.cpp`) picks up the slack until a later frame retries. **Investigation of the current code shows this doesn't actually work**, independent of whether Part 2's emdawnwebgpu bug is ever fixed:

- This fork sets `API_TRAIT_REQUIRES_SYNCHRONOUS_PIPELINE_COMPILATION = 1` for WebGPU (`rendering_device_driver_webgpu.cpp:11387`), because WGPU object handles live in a per-thread JS lookup table and a background `WorkerThreadPool` thread can't see handles created on the device-owning thread (documented at `servers/rendering/rendering_device_driver.h:1051-1072`). This means `PipelineHashMapRD::compile_pipeline()` (`servers/rendering/renderer_rd/pipeline_hash_map_rd.h:150-160`) always runs the creation function **inline, synchronously, on the calling thread** for this backend — the `p_wait_for_compilation=false` fast path in `get_pipeline()` still pays the full Tint cost before returning; it just doesn't wait around afterward.
- `PipelineHashMapRD::compilation_set` (the idempotency guard preventing duplicate compile requests) has **no "retry later" state**. Once a hash is marked as requested, `compile_pipeline()` is a permanent no-op for that hash unless `clear_pipelines()` wipes the whole map. If a deferred/nullptr Tint result is threaded through as a driver failure so `add_compiled_pipeline()` is skipped, that specialized variant is stuck on the ubershader **forever**, not "until next frame."
- Today, a `_create_module_with_spec_constants()` nullptr is *not* surfaced as a failure at all: `render_pipeline_create()` (`rendering_device_driver_webgpu.cpp:10043`, nullptr-handling at ~10120-10133) silently substitutes the base/unspecialized shader module and still returns a successful `PipelineID`, which then gets cached forever under the specialized hash. This is a safe, working fallback for genuine Tint failures, but it's a one-way permanent quality downgrade, not a deferral — repurposing it to mean "budget exceeded, try again later" would conflate the two outcomes and needs to be split into a distinct new state.

**Implication:** shipping even "just" Part 1 safely requires changes to `PipelineHashMapRD` — shared code used by every RD backend (Vulkan/Metal/D3D12 too), not WebGPU-specific — to add a real deferred/retry state. That's a materially larger and riskier change than this doc's original framing ("cherry-pick the worker, low risk, proven code") suggested, and carries regression risk for other backends that this session could not validate (no way to build/test Vulkan or Metal changes from this environment). Combined with 10.1 (the worker code has to be rebuilt from scratch anyway, not cherry-picked), the honest cost/risk of this plan is significantly higher than previously documented.

**Recommendation if revisited:** scope the `PipelineHashMapRD` retry-state change as its own reviewed, cross-backend-tested PR *before* building the worker, so the prerequisite is validated independently of the WebGPU-specific worker plumbing. Until then, prefer lower-risk mitigations that don't touch shared pipeline-cache code — e.g. auditing export-time WGSL baking coverage (`rendering_shader_container_webgpu.cpp` / `wgsl_bake_subprocess.*`) for gaps that force otherwise-avoidable runtime Tint misses.
